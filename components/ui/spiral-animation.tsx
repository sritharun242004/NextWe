...
class AnimationController {
    ...
    private drawTrail(t1: number) {
        for (let i = 0; i < this.trailLength; i++) {
            const f = this.map(i, 0, this.trailLength - 1, 1, 0)
            const sw = (2.0 + 3.0 * Math.sin(Math.PI * t1)) * f

            this.ctx.fillStyle = '#a0ff4a'
            this.ctx.lineWidth = sw
...
class Star {
    ...
    constructor(cameraZ: number, cameraTravelDistance: number) {
        ...
        this.finalScale = 0.7 + Math.random() * 0.6 // Between 0.7-1.3 final size
    ...
    render(p: number, controller: AnimationController) {
        ...
        if (q > 0) {
            ...
            if (displacementProgress < 0.3) {
                // Initial stage: straight line movement (30%)
                ...
                // Base position (70% straight line distance)
                const baseX = spiralPos.x + this.dx * 0.7;
                const baseY = spiralPos.y + this.dy * 0.7;

                // Final position with increased spiral effect
            ...
            // Particle size animation: normal initially, slightly larger in middle, finally adjust according to finalScale
            let sizeMultiplier;
